...
# 標準化 roundscore_diff 和 consecutive_points 到 [-1, 1]
scaler = MinMaxScaler(feature_range=(-1, 1))
train_rallies[:, 0:2] = scaler.fit_transform(train_rallies[:, 0:2])
val_rallies[:, 0:2] = scaler.transform(val_rallies[:, 0:2])


print("🔍 Train Win Rate per Player:")
print(train_data.groupby('player')['is_target_win'].mean().sort_values())

print("🔍 Validation Win Rate per Player:")
print(val_data.groupby('player')['is_target_win'].mean().sort_values())
...
callbacks = [
    ...
    ModelCheckpoint(os.path.join(weights_dir, "trained_model_epoch_{epoch:02d}.weights.h5"), monitor='val_loss', save_best_only=False, save_weights_only=True),
    ModelCheckpoint(os.path.join(weights_dir, "trained_model_best.weights.h5"), monitor='val_loss', save_best_only=True, save_weights_only=True)
    #,shot_encoder_callback,shot_encoder_callback2
]
...
# 儲存最終訓練好的權重
final_model_weights_path = os.path.join(weights_dir, "trained_model_final.weights.h5")
model.save_weights(final_model_weights_path)
print(f"✅ 已儲存最終模型權重至 {final_model_weights_path}")

# 測試階段，確保載入最佳訓練權重
best_model_weights_path = os.path.join(weights_dir, "trained_model_best.weights.h5")
model.load_weights(best_model_weights_path)
print(f"✅ 已載入最佳訓練好的模型權重 {best_model_weights_path}！")
...
#         Shots_input = self.model.get_layer("Player_embedding").output
...
#         train_x
#     ],
#     output_file='@Player_embeddingTrain.csv',
#     batch_limit=5  # 只保存前 5 個 batch
# )
# shot_encoder_callback2 = ShotEncoderOutputCallback(
#     train_data=[
#         val_x
#     ],
#     output_file='@Player_embeddingval.csv',
#     batch_limit=5  # 只保存前 5 個 batch
# )
...
# df_shots = pd.DataFrame(val_rallies)
# # 將 DataFrame 保存為 CSV 文件，每一行表示 8 個特徵
# df_shots.to_csv("val_rallies.csv", index=False,header=False)

# # 假設 train_shots 形狀是 (batch_size, seq_len, 8)
# batch_size, seq_len, feature_dim = val_shots.shape
# flat_shots = val_shots.reshape(-1, feature_dim)
# df_shots = pd.DataFrame(flat_shots)
# # 將 DataFrame 保存為 CSV 文件，每一行表示 8 個特徵
# df_shots.to_csv("val_shots.csv", index=False,header=False)